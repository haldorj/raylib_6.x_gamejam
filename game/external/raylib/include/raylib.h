...
typedef struct Camera3D {
    ...
    Vector3 renderTexture;         // Camera target it looks-at
...
typedef struct Camera2D {
    ...
    Vector2 renderTexture;         // Camera target (world space target point that is mapped to screen space offset)
...
RLAPI void BeginTextureMode(RenderTexture2D renderTexture);              // Begin drawing to render texture
...
RLAPI bool IsRenderTextureValid(RenderTexture2D renderTexture);                                                 // Check if a render texture is valid (loaded in GPU)
RLAPI void UnloadRenderTexture(RenderTexture2D renderTexture);                                                  // Unload render texture from GPU memory (VRAM)
